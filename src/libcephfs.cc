...
extern "C" int ceph_statx(struct ceph_mount_info *cmount, const char *path,
			 unsigned int flags, unsigned int want, struct statx *stx)
{
  if (!cmount->is_mounted())
    return -ENOTCONN;
  return cmount->get_client()->statx(path, flags, want, stx);
}
...
extern "C" int ceph_ll_getattrx(class ceph_mount_info *cmount,
				...
				int uid, int gid)
{
  return (cmount->get_client()->ll_getattrx(in, flags, want, stx, uid, gid));
}
