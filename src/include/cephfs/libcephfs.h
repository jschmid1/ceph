 ...
 * @param want bitfield of STATX_* flags showing designed attributes
 * @param stx the statx struct that will be filled in with the file's statistics.
 * @returns 0 on success or negative error code on failure.
 */
int ceph_statx(struct ceph_mount_info *cmount, const char *path, unsigned int flags,
	       unsigned int want, struct statx *stx);
 ...
 * @returns 0 on success or negative error code on failure.
 */
int ceph_setattrx(struct ceph_mount_info *cmount, const char *relpath, struct statx *stx, int mask);
...
int ceph_ll_getattrx(struct ceph_mount_info *cmount, struct Inode *in,
		    unsigned int mask, unsigned int want, struct statx *stx,
		    int uid, int gid);
...
int ceph_ll_setattrx(struct ceph_mount_info *cmount, struct Inode *in,
		    struct statx *stx, int mask, int uid, int gid);
