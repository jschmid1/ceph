...
void Client::handle_cap_trunc(MetaSession *session, Inode *in, MClientCaps *m)
{
  ...
  update_inode_file_bits(in, m->get_truncate_seq(), m->get_truncate_size(),
			 m->get_change_attr(), m->get_size(), m->get_time_warp_seq(),
...
int Client::may_setattr(Inode *in, struct statx *stx, int mask, int uid, int gid)
{
  if (uid < 0)
    uid = get_uid();
  if (gid < 0)
...
int Client::_do_setattr(Inode *in, struct statx *stx, int mask, int uid, int gid,
			InodeRef *inp)
{
...
void Client::stat_to_statx(struct stat *st, struct statx *stx)
{
...
int Client::__setattrx(Inode *in, struct statx *stx, int mask, int uid, int gid,
		     InodeRef *inp)
{
...
int Client::_setattrx(InodeRef &in, struct statx *stx, int mask)
{
...
int Client::setattrx(const char *relpath, struct statx *stx, int mask)
{
  Mutex::Locker lock(client_lock);
...
unsigned Client::statx_to_mask(unsigned int flags, unsigned int want)
{
  ...
  mask |= CEPH_CAP_PIN;
  if (want & (STATX_MODE|STATX_UID|STATX_GID|STATX_RDEV|STATX_BTIME))
...
int Client::statx(const char *relpath, unsigned int flags, unsigned int want,
		  struct statx *stx)
{
...
void Client::fill_statx(Inode *in, unsigned int mask, struct statx *stx)
{
  ...
  stx->stx_mask |= (STATX_INO|STATX_RDEV);
  ...
  if (mask & CEPH_CAP_AUTH_SHARED) {
    ...
    stx->stx_mask |= (STATX_MODE|STATX_UID|STATX_GID|STATX_BTIME);
  }
...
int Client::ll_getattrx(Inode *in, unsigned int flags, unsigned int want,
...
int Client::_ll_setattrx(Inode *in, struct statx *stx, int mask, int uid,
...
int Client::ll_setattrx(Inode *in, struct statx *stx, int mask, int uid,
		       int gid)
{
  InodeRef target(in);
